...
def add_component(name, element_type):
    ...
    print(element_header_path)
    lib_dir_path = os.path.join(os.path.dirname(element_header_path),os.path.pardir)
...
def compileComponent(component):
    ...
    ret_code = subprocess.call(
        ['make', f'VERILOG_MACROS={component["macros_verilog"]}', '-C', component['lib_dir_path']], stderr=subprocess.STDOUT)
    assert ret_code == 0
